...
def delta_A_M_solver(Phi, Pi, x, t):
    ...
    integrando  = rho*np.exp(-delta) - Pib**2
    integrando *= tan2x
    ...
    # Calculamos la masa:
    if rank==size-1:
        M = np.exp(delta[Nx])*integral[Nx] - 0.5*np.pi*Pib**2
    else:
        M = 0.
...
def k_Pi_solver(A, delta, Phi, Pi, x, t):
    ...
    if rank==size-1:
        ...
        # Punto i=Nx:
        #dPhiNx   = dPi0(t) # Forzamos el valor correcto en la frontera obtenido de la expansión en serie
        dPhiNx   = 0.
...
if __name__ == '__main__':
    ...
    # Archivos de salida:
    fout_evolucion = 'DC_evolucion_Source_estatico_%4.2fx%i.%i.nc' % (alpha, points, rank)
    fout_series    = 'DC_series_Source_estatico_%4.2fx%i.%i.nc' % (alpha, points, rank)
